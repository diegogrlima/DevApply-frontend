...
const timelineColors: Record<string, string> = {
  'Currículo enviado': 'bg-[#1DB954]',
  'Currículo em análise': 'bg-blue-500',
  'Convite para entrevista técnica': 'bg-emerald-500',
  'Entrevista técnica agendada': 'bg-amber-500',
  'Proposta': 'bg-[#333]',
}
...
export default function ApplicationDetails() {
  ...
  const app = applicationsData[id || '1'] || applicationsData['1']
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <span className={`px-4 py-2 rounded-full text-sm font-medium ${
              app.status === 'Em andamento' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'
            }`}>
              {app.stage}
            </span>
              ...
              {app.timeline.map((item, index) => (
                <div key={index} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-3 h-3 rounded-full ${item.done ? (timelineColors[item.event] || 'bg-[#1DB954]') : 'bg-[#333]'}`} />
